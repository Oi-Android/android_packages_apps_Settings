...
import com.android.settings.oi.utils.oiSwitchPreference;
import com.android.settings.oi.utils.SeekBarPreferenceCham;
import com.android.settings.oi.sensor.ShakeSensorManager;
...
public class AmbientSettings extends SettingsPreferenceFragment implements
    ...
    private oiSwitchPreference mDozeTimeMode;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        int dozePulseIn = Settings.OI.getInt(getContentResolver(),
                Settings.OI.DOZE_PULSE_DURATION_IN, 900);
        ...
        int dozePulseInPickup = Settings.OI.getInt(getContentResolver(),
                Settings.OI.DOZE_PULSE_DURATION_IN_PICKUP, 300);
        ...
        int dozePulseInIntent = Settings.OI.getInt(getContentResolver(),
                Settings.OI.DOZE_PULSE_DURATION_IN_INTENT, 300);
        ...
        int dozePulseVisible = Settings.OI.getInt(getContentResolver(),
                Settings.OI.DOZE_PULSE_DURATION_IN, 3000);
        ...
        int dozePulseOut = Settings.OI.getInt(getContentResolver(),
                Settings.OI.DOZE_PULSE_DURATION_IN, 600);
        ...
        if (isAccelerometerAvailable(activity)) {
            ...
            mDozeTimeMode = (oiSwitchPreference) findPreference(KEY_DOZE_TIME_MODE);
    ...
    private void updateDozeListMode() {
        final boolean pocketMode = Settings.OI.getInt(getContentResolver(),
                   Settings.OI.DOZE_POCKET_MODE, 0) != 0;
        final boolean shakeMode = Settings.OI.getInt(getContentResolver(),
                   Settings.OI.DOZE_SHAKE_MODE, 0) != 0;
        final boolean pulseMode = Settings.OI.getInt(getContentResolver(),
                   Settings.OI.DOZE_PULSE_ON_NOTIFICATIONS, 1) != 0;
        int listMode = 6;
    ...
    private void appliedAccTest() {
        Settings.OI.putInt(getContentResolver(),
                    Settings.OI.DOZE_SHAKE_ACC_THRESHOLD, mAccValue);
    ...
    private void enableShakeThreshold(boolean enabled) {
        final boolean writeMode = Settings.OI.getInt(getContentResolver(),
                   Settings.OI.DOZE_OVERWRITE_VALUE, 0) != 0;
    ...
    private void updateDozeListModeValue(int listMode) {
        if (listMode == 1) {
            ...
            Settings.OI.putInt(getContentResolver(),
                   Settings.OI.DOZE_POCKET_MODE, 1);
            Settings.OI.putInt(getContentResolver(),
                   Settings.OI.DOZE_SHAKE_MODE, 1);
            Settings.OI.putInt(getContentResolver(),
                   Settings.OI.DOZE_PULSE_ON_NOTIFICATIONS, 1);
        } else if (listMode == 2) {
            enableShakeThreshold(true);
            enabledTimeMode(true);
            Settings.OI.putInt(getContentResolver(),
            ...
            Settings.OI.putInt(getContentResolver(),
                   Settings.OI.DOZE_SHAKE_MODE, 1);
            Settings.OI.putInt(getContentResolver(),
                   Settings.OI.DOZE_PULSE_ON_NOTIFICATIONS, 0);
        } else if (listMode == 3) {
            enableShakeThreshold(false);
            enabledTimeMode(true);
            Settings.OI.putInt(getContentResolver(),
            ...
            Settings.OI.putInt(getContentResolver(),
                   Settings.OI.DOZE_POCKET_MODE, 0);
            Settings.OI.putInt(getContentResolver(),
                   Settings.OI.DOZE_SHAKE_MODE, 1);
            Settings.OI.putInt(getContentResolver(),
                   Settings.OI.DOZE_PULSE_ON_NOTIFICATIONS, 1);
    ...
    private void updateDozeOptions() {
        if (mDozePulseIn != null) {
            final int statusDozePulseIn = Settings.OI.getInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_IN, 900);
            mDozePulseIn.setValue(statusDozePulseIn);
        }
        if (mDozePulseInPickup != null) {
            final int statusDozePulseInPickup = Settings.OI.getInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_IN_PICKUP, 300);
            mDozePulseInPickup.setValue(statusDozePulseInPickup);
        }
        if (mDozePulseInIntent != null) {
            final int statusDozePulseInIntent = Settings.OI.getInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_IN_INTENT, 300);
            mDozePulseInIntent.setValue(statusDozePulseInIntent);
        }
        if (mDozePulseVisible != null) {
            final int statusDozePulseVisible = Settings.OI.getInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_VISIBLE, 3000);
            mDozePulseVisible.setValue(statusDozePulseVisible);
        }
        if (mDozePulseOut != null) {
            final int statusDozePulseOut = Settings.OI.getInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_OUT, 600);
            mDozePulseOut.setValue(statusDozePulseOut);
        }
        if (mDozeShakeThreshold != null) {
            mAccValue = Settings.OI.getInt(getContentResolver(),
                    Settings.OI.DOZE_SHAKE_ACC_THRESHOLD, 10);
            mDozeShakeThreshold.setValue(String.valueOf(mAccValue));
            int index = mDozeShakeThreshold.findIndexOfValue(String.valueOf(mAccValue));
            if (index != -1) {
                mDozeShakeThreshold.setSummary(mDozeShakeThreshold.getEntries()[index]);
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        ...
        if (preference == mDozePulseIn) {
            ...
            Settings.OI.putInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_IN, dozePulseIn);
        }
        if (preference == mDozePulseInPickup) {
            int dozePulseInPickup = (Integer) objValue;
            Settings.OI.putInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_IN_PICKUP, dozePulseInPickup);
        }
        if (preference == mDozePulseInIntent) {
            int dozePulseInIntent = (Integer) objValue;
            Settings.OI.putInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_IN_INTENT, dozePulseInIntent);
        }
        if (preference == mDozePulseVisible) {
            int dozePulseVisible = (Integer) objValue;
            Settings.OI.putInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_VISIBLE, dozePulseVisible);
        }
        if (preference == mDozePulseOut) {
            int dozePulseOut = (Integer) objValue;
            Settings.OI.putInt(getContentResolver(),
                    Settings.OI.DOZE_PULSE_DURATION_OUT, dozePulseOut);
        }
