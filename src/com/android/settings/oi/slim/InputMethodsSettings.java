...
package com.android.settings.oi.slim;
...
public class InputMethodsSettings extends SettingsPreferenceFragment implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mDisableFullscreenKeyboard.setChecked(Settings.OI.getInt(getContentResolver(),
                Settings.OI.DISABLE_FULLSCREEN_KEYBOARD, 0) == 1);
        ...
        mKeyboardRotationToggle.setChecked(Settings.OI.getInt(getContentResolver(),
                Settings.OI.KEYBOARD_ROTATION_TIMEOUT, 0) > 0);
        ...
        updateRotationTimeout(Settings.OI.getInt(
                getContentResolver(), Settings.OI.KEYBOARD_ROTATION_TIMEOUT,
                KEYBOARD_ROTATION_TIMEOUT_DEFAULT));

        mShowEnterKey = (SwitchPreference) findPreference(SHOW_ENTER_KEY);
        mShowEnterKey.setChecked(Settings.OI.getInt(getContentResolver(),
                Settings.OI.FORMAL_TEXT_INPUT, 0) == 1);
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference == mDisableFullscreenKeyboard) {
            Settings.OI.putInt(getContentResolver(),
                    Settings.OI.DISABLE_FULLSCREEN_KEYBOARD,  (Boolean) objValue ? 1 : 0);
            return true;
            ...
            Settings.OI.putInt(getContentResolver(),
                    Settings.OI.KEYBOARD_ROTATION_TIMEOUT,
            ...
            Settings.OI.putInt(getContentResolver(),
                Settings.OI.FORMAL_TEXT_INPUT, (Boolean) objValue ? 1 : 0);
            return true;
        } else if (preference == mKeyboardRotationTimeout) {
            int timeout = Integer.parseInt((String) objValue);
            Settings.OI.putInt(getContentResolver(),
                    Settings.OI.KEYBOARD_ROTATION_TIMEOUT, timeout);
