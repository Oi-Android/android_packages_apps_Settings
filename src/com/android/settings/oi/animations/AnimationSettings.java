...
package com.android.settings.oi;
...
import com.android.settings.oi.utils.AppMultiSelectListPreference;
import com.android.settings.oi.utils.SeekBarPreferenceCham;
import com.android.settings.SettingsPreferenceFragment;
import com.android.settings.R;

import com.android.internal.util.oi.AwesomeAnimationHelper;
...
public class AnimationSettings extends SettingsPreferenceFragment  implements
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mAnimExitOnly.setChecked(Settings.OI.getInt(mResolver,
                Settings.OI.ANIMATION_CONTROLS_EXIT_ONLY, 0) == 1);
        ...
        mAnimReverseExit.setChecked(Settings.OI.getInt(mResolver,
                Settings.OI.ANIMATION_CONTROLS_REVERSE_EXIT, 0) == 1);
        ...
        int defaultDuration = Settings.OI.getInt(mResolver,
                Settings.OI.ANIMATION_CONTROLS_DURATION, 25);
        ...
        int listviewDuration = Settings.OI.getInt(mResolver,
                Settings.OI.LISTVIEW_DURATION, 25);
        ...
        mToastAnimation.setValue(Integer.toString(Settings.OI.getInt(mResolver,
             Settings.OI.ANIMATION_TOAST, 1)));
    ...
    private void resetAllSettings() {
        ...
        Settings.OI.putString(getContentResolver(),
                Settings.OI.LISTVIEW_ANIMATION_EXCLUDED_APPS, "");
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        if (preference == mAnimExitOnly) {
            ...
            Settings.OI.putInt(mResolver,
                    Settings.OI.ANIMATION_CONTROLS_EXIT_ONLY,
                    value ? 1 : 0);
            updateRevExitAnim();
        } else if (preference == mAnimReverseExit) {
            boolean value = (Boolean) objValue;
            Settings.OI.putInt(mResolver,
                    Settings.OI.ANIMATION_CONTROLS_REVERSE_EXIT,
                    value ? 1 : 0);
        } else if (preference == mActivityOpenPref) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[0], val);
            mActivityOpenPref.setSummary(getProperSummary(mActivityOpenPref));
        } else if (preference == mActivityClosePref) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[1], val);
            mActivityClosePref.setSummary(getProperSummary(mActivityClosePref));
        } else if (preference == mTaskOpenPref) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[2], val);
            mTaskOpenPref.setSummary(getProperSummary(mTaskOpenPref));
        } else if (preference == mTaskClosePref) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[3], val);
            mTaskClosePref.setSummary(getProperSummary(mTaskClosePref));
        } else if (preference == mTaskMoveToFrontPref) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[4], val);
            mTaskMoveToFrontPref.setSummary(getProperSummary(mTaskMoveToFrontPref));
        } else if (preference == mTaskMoveToBackPref) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[5], val);
            mTaskMoveToBackPref.setSummary(getProperSummary(mTaskMoveToBackPref));
        } else if (preference == mWallpaperOpen) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[6], val);
            mWallpaperOpen.setSummary(getProperSummary(mWallpaperOpen));
        } else if (preference == mWallpaperClose) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[7], val);
            mWallpaperClose.setSummary(getProperSummary(mWallpaperClose));
        } else if (preference == mWallpaperIntraOpen) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[8], val);
            mWallpaperIntraOpen.setSummary(getProperSummary(mWallpaperIntraOpen));
        } else if (preference == mWallpaperIntraClose) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[9], val);
            mWallpaperIntraClose.setSummary(getProperSummary(mWallpaperIntraClose));
        } else if (preference == mTaskOpenBehind) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ACTIVITY_ANIMATION_CONTROLS[10], val);
            mTaskOpenBehind.setSummary(getProperSummary(mTaskOpenBehind));
        } else if (preference == mAnimationDuration) {
            int val = ((Integer)objValue).intValue();
            Settings.OI.putInt(mResolver,
                    Settings.OI.ANIMATION_CONTROLS_DURATION,
                    val);
        } else if (preference == mListViewAnimation) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver, Settings.OI.LISTVIEW_ANIMATION, val);
            mListViewAnimation.setSummary(getListAnimationName(val));
        } else if (preference == mListViewCache) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver, Settings.OI.LISTVIEW_ANIMATION_CACHE, val);
            mListViewCache.setSummary(getListCacheName(val));
        } else if (preference == mListViewInterpolator) {
            int val = Integer.parseInt((String) objValue);
            Settings.OI.putInt(mResolver, Settings.OI.LISTVIEW_INTERPOLATOR, val);
            ...
            int val = ((Integer)objValue).intValue();
            Settings.OI.putInt(mResolver,
                    Settings.OI.LISTVIEW_DURATION,
                    val);
        } else if (preference == mToastAnimation) {
            int val = Integer.parseInt((String) objValue);
            int index = mToastAnimation.findIndexOfValue((String) objValue);
            Settings.OI.putInt(mResolver,
                    Settings.OI.ANIMATION_TOAST, val);
    ...
    private void setProperVal(Preference preference, int val) {
        ...
        if (preference == mActivityOpenPref) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[0];
        } else if (preference == mActivityClosePref) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[1];
        } else if (preference == mTaskOpenPref) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[2];
        } else if (preference == mTaskClosePref) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[3];
        } else if (preference == mTaskMoveToFrontPref) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[4];
        } else if (preference == mTaskMoveToBackPref) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[5];
        } else if (preference == mWallpaperOpen) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[6];
        } else if (preference == mWallpaperClose) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[7];
        } else if (preference == mWallpaperIntraOpen) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[8];
        } else if (preference == mWallpaperIntraClose) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[9];
        } else if (preference == mTaskOpenBehind) {
            mString = Settings.OI.ACTIVITY_ANIMATION_CONTROLS[10];
        } else if (preference == mListViewAnimation) {
            mString = Settings.OI.LISTVIEW_ANIMATION;
        } else if (preference == mListViewCache) {
            mString = Settings.OI.LISTVIEW_ANIMATION_CACHE;
        } else if (preference == mListViewInterpolator) {
            mString = Settings.OI.LISTVIEW_INTERPOLATOR;
        } else if (preference == mListViewDuration) {
            mString = Settings.OI.LISTVIEW_DURATION;
        }

        Settings.OI.putInt(mResolver, mString, val);
    }
    ...
    private String getProperSummary(Preference preference) {
        ...
        String mNum = Settings.OI.getString(mResolver, mString);
    ...
    private String getProperVal(Preference preference) {
        ...
        return Settings.OI.getString(mResolver, mString);
    }

    private void updateRevExitAnim() {
        boolean enabled = Settings.OI.getInt(mResolver,
                Settings.OI.ANIMATION_CONTROLS_EXIT_ONLY, 0) == 1;
    ...
    private Set<String> getExcludedApps() {
        String excluded = Settings.OI.getString(getContentResolver(),
                Settings.OI.LISTVIEW_ANIMATION_EXCLUDED_APPS);
    ...
    private void storeExcludedApps(Set<String> values) {
        ...
        Settings.OI.putString(getContentResolver(),
                Settings.OI.LISTVIEW_ANIMATION_EXCLUDED_APPS, builder.toString());
    }
}
